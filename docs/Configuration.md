...
4. Vault                      secret/ and database/ — credentials only, fetched at bootstrap
5. Environment variables      from docker/compose/.env, via the compose files
```

Layers 3 and 4 divide one job and never overlap:

> **Configuration is a value that could appear in a screenshot. A secret is a value that could not.**

A port, a pool size, a TTL → Consul KV. A password, a token, an access key → Vault. Nothing lives in
both; a value with two sources of truth will eventually disagree, and the copy that wins is whichever
one nobody checked. See [`docs/Vault.md`](Vault.md).

Note that layer 5 still outranks layer 4 for any key it actually defines. That is why the services no
longer receive `ORDER_DB_PASSWORD` or `REDIS_PASSWORD` in their environment at all — an env var that
shadowed a Vault value would produce a service that looks wired to Vault and is not.

| File | Owns | Tracked in git |
| --- | --- | --- |
| `docker/compose/.env` | Deployment variables: ports, image pins, limits, and the **infrastructure** credentials Keycloak/Grafana/Redis/MinIO/PostgreSQL/Oracle read at boot | **No** |
| `docker/compose/.env.example` | The template and the list of every variable that exists | Yes |
| `docker/compose/.vault-keys.json` | Vault's unseal keys and root token — generated by `./scripts/vault.sh init` | **No** |
| `docker/compose/.env.vault.<service>` | Per-service AppRole credentials — generated by `./scripts/vault.sh seed` | **No** |
...
| `DB_DEFAULT_TTL` in `scripts/vault.sh` **downward** | `spring.datasource.hikari.max-lifetime` downward, so it stays below | The pool hands out connections belonging to a PostgreSQL user Vault already dropped. The errors start minutes after the expiry, so nothing points at the change that caused them |
| `DB_MAX_TTL` **upward** | `max_lease_ttl` in `infrastructure/vault/config/vault.hcl` | A mount cannot outlive its server's cap. Vault truncates and merely *warns*, so the role says 24h and behaves as 8h |
| `SPRING_PROFILES_ACTIVE` to a new value | Nothing — the policies already grant `secret/data/<service>/*` | Without that wildcard, startup fails with a 403 naming `secret/data/<service>/<profile>`, a key that was never meant to exist |
| A new secret in `application.yml` | Write it to Vault under its **exact Spring property name** | The property silently falls back to the `${VAR:default}` in the file, and the default is a working lab credential — so it looks correct |
...
Since step 20 there are two homes, and which one a credential lives in depends on whether the thing
that needs it can **ask** for it.

| Consumer | Gets credentials from | Because |
| --- | --- | --- |
| `order-service`, `inventory-service` | **Vault** | Spring Cloud Vault authenticates with an AppRole at bootstrap |
| Keycloak, Grafana, Redis, MinIO, PostgreSQL, Oracle | `.env` | They read environment variables at container start and cannot query anything |

`.env` is git-ignored; `.env.example` is tracked and carries obvious placeholders that must never
become real values.

| Account | Scope | Source |
| --- | --- | --- |
| `v-approle-order-se-…` | `orderdb`, via the `order_readwrite` group role | **Vault, per lease** — created on demand, dropped when the lease ends |
| `order_user` | `orderdb` — owns the schema, used by Flyway only | Vault KV (`secret/order-service`) |
| `inventory_user` | The Oracle PDB app schema | Vault KV (`secret/inventory-service`) |
| `MINIO_APP_USER` | The invoice bucket only. MinIO root is never used by a service | Vault KV (`secret/order-service`) |
| Redis password | — | Vault KV (`secret/application`) |
| `keycloak_user` | `keycloakdb` only — neither can read the other's data | `.env` |
| `MONITOR_DB_USER`, `ORACLE_MONITOR_USER` | **Read-only**, for the exporters | `.env` |
| PostgreSQL superuser | Everything — it is what Vault uses to create the dynamic users | `.env` |

Flyway keeping a **static** credential while the runtime pool gets a dynamic one is deliberate and
load-bearing: migrations create objects that acquire an owner, and an owner on a one-hour lease takes
the schema with it. [`docs/Vault.md` §7](Vault.md#7-dynamic-postgresql-credentials) has the detail.
